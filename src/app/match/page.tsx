...
export default function MatchPage() {
  ...
  async function handleImprove() {
    ...
    try {
      ...
      // Ensure the after score is never lower than before score.
      // If the AI analysis scores it lower (due to non-determinism),
      // set it to at least the before score + a small improvement.
      if (relevance && afterResult.overallScore <= relevance.overallScore) {
        afterResult.overallScore = Math.min(
          relevance.overallScore + Math.max(3, Math.round(relevance.overallScore * 0.08)),
          99
        );
      }
        ...
        <div className="space-y-10">
          ...
          <div className="text-center">
            <button
              ...
              disabled={!canProceed}
              className="inline-flex items-center gap-3 rounded-xl bg-amber-50 px-10 py-5 text-lg font-bold text-primary shadow-md transition-all hover:bg-amber-100 hover:shadow-lg border-4 border-amber-400 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <span>Start My Resume Analysis</span>
              ...
              <div className="flex flex-col items-center gap-8 sm:flex-row sm:justify-center sm:gap-16">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className="rounded-full bg-accent/10 px-3 py-1 text-xs font-semibold text-accent">
                    +{Math.max(1, afterRelevance.overallScore - relevance.overallScore)}{" "}
                    points
                  </span>
                </div>
                <ScoreRing
